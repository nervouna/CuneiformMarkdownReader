...
grep -F 'return .webview' Sources/SimpleMarkdownPreviewerApp/PreviewRendererMode.swift >/dev/null
grep -F 'Cuneiform renderer: ${CUNEIFORM_RENDERER:-webview}' scripts/measure_startup.sh >/dev/null
